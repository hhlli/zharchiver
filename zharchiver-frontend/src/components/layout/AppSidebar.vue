  ...
  <aside class="flex inset-y-0 left-0 z-50 w-64 md:w-52 bg-[#f5f5f7] border-r border-gray-200 p-3 flex-col justify-between flex-shrink-0 h-full overflow-y-auto">
    ...
    <div v-if="store.currentView === 'home'" class="space-y-6">
      <div>
        <nav class="space-y-1">
          <button 
            ...
            :class="['w-full flex items-center space-x-2.5 px-3 py-1.5 rounded-lg text-xs font-medium transition cursor-pointer', store.activeCategory === 'all' && !store.currentAnswer ? 'bg-blue-600 text-white shadow-sm' : 'text-gray-700 hover:bg-gray-200/60']"
          >
      ...
      <div v-if="store.tags && store.tags.length > 0">
        <div class="text-[11px] font-semibold text-gray-400 px-3 mb-2 tracking-wider">标签</div>
        <div class="space-y-1">
          <button
    ...
    <div v-else-if="store.currentView === 'settings'" class="space-y-6">
      <div>
        <button 
          ...
          class="w-full flex items-center space-x-2 px-3 py-2 text-xs font-medium text-gray-500 hover:text-gray-800 hover:bg-gray-200/60 rounded-lg transition mb-4 cursor-pointer"
        >
        ...
        <nav class="space-y-1">
          <button 
            ...
            :class="['w-full text-left px-3 py-1.5 rounded-lg text-xs font-medium transition cursor-pointer', store.activeSetting === 'security' ? 'bg-gray-200/80 text-gray-900 shadow-sm' : 'text-gray-600 hover:bg-gray-200/50']"
          >账户安全</button>
          <button 
            @click="store.activeSetting = 'auth'"
            :class="['w-full text-left px-3 py-1.5 rounded-lg text-xs font-medium transition cursor-pointer', store.activeSetting === 'auth' ? 'bg-gray-200/80 text-gray-900 shadow-sm' : 'text-gray-600 hover:bg-gray-200/50']"
          >平台鉴权</button>
          <button 
            @click="store.activeSetting = 'data'"
            :class="['w-full text-left px-3 py-1.5 rounded-lg text-xs font-medium transition cursor-pointer', store.activeSetting === 'data' ? 'bg-gray-200/80 text-gray-900 shadow-sm' : 'text-gray-600 hover:bg-gray-200/50']"
          >数据管理</button>
          <button 
            @click="store.activeSetting = 'display'"
            :class="['w-full text-left px-3 py-1.5 rounded-lg text-xs font-medium transition cursor-pointer', store.activeSetting === 'display' ? 'bg-gray-200/80 text-gray-900 shadow-sm' : 'text-gray-600 hover:bg-gray-200/50']"
          >偏好设置</button>
          <button 
            @click="store.activeSetting = 'ai'"
            :class="['w-full text-left px-3 py-1.5 rounded-lg text-xs font-medium transition cursor-pointer', store.activeSetting === 'ai' ? 'bg-gray-200/80 text-gray-900 shadow-sm' : 'text-gray-600 hover:bg-gray-200/50']"
          >工具</button>
          <button 
            @click="store.activeSetting = 'logs'"
            :class="['w-full text-left px-3 py-1.5 rounded-lg text-xs font-medium transition cursor-pointer', store.activeSetting === 'logs' ? 'bg-gray-200/80 text-gray-900 shadow-sm' : 'text-gray-600 hover:bg-gray-200/50']"
          >日志中心</button>
        </nav>
      </div>
    </div>
    ...
    <div class="flex items-center justify-between mt-auto">
      <div class="flex items-center space-x-1">
        ...
        <div class="text-[11px] text-gray-400 font-mono hidden md:block">ZHArchiver v1.2</div>
      </div>
