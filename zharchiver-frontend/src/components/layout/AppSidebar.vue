<template>
  <aside :class="['hidden md:flex inset-y-0 left-0 z-50 w-64 md:w-52 bg-[#f5f5f7] border-r border-gray-200 p-3 flex-col justify-between flex-shrink-0 h-full overflow-y-auto', isDesktopOpen ? 'flex' : 'hidden']">
      ...
      <div>
        <nav class="space-y-1">
          <button 
            ...
            :class="['w-full flex items-center space-x-2.5 px-3 py-1.5 rounded-lg text-xs font-medium transition cursor-pointer', activeCategory === 'all' && !hasCurrentAnswer ? 'bg-blue-600 text-white shadow-sm' : 'text-gray-700 hover:bg-gray-200/60']"
          >
      ...
      <div v-if="tags && tags.length > 0">
        ...
        <div class="space-y-1">
          <button
            ...
            :class="['w-full flex items-center space-x-2 px-3 py-1.5 rounded-lg text-xs font-medium transition cursor-pointer', activeCategory === tag.name && !hasCurrentAnswer ? 'bg-blue-100 text-blue-700 shadow-sm' : 'text-gray-600 hover:bg-gray-200/50']"
          >
    ...
    <div v-else-if="currentView === 'settings'" class="space-y-6">
      <div>
        ...
        <nav class="space-y-1">
          ...
          >工具</button>
          <button 
            @click="handleSettingClick('logs')"
            :class="['w-full text-left px-3 py-1.5 rounded-lg text-xs font-medium transition cursor-pointer', activeSetting === 'logs' ? 'bg-gray-200/80 text-gray-900 shadow-sm' : 'text-gray-600 hover:bg-gray-200/50']"
          >日志中心</button>
        </nav>
      </div>
    </div>
    ...
    <div class="flex items-center justify-between mt-auto">
      <div class="flex items-center space-x-1">
        <button @click="$emit('collapse-desktop')" class="hidden md:flex p-1 rounded text-gray-400 hover:text-gray-700 hover:bg-gray-200 transition cursor-pointer items-center justify-center" title="收起边栏">
